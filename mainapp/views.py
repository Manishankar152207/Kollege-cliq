...
from college.models import CollegeBasicInfo, CollegeAffiliation
    ...
    if request.method == 'POST':
        if RegisteredUsers.objects.filter(username=request.POST['username'].lower()):
            ...
            if not request.POST['password']:
                ...
                print(user)
                if user.password == make_password(request.POST['password']):  # manke_password
                    request.session['college_id']= str(user.user_id)
                    request.session['college_email']= user.email
                    return JsonResponse({"message":"ok",'college_id':user.user_id})
                else:
                    return JsonResponse({"message":"Incorrect username or password."})
        else:
...
def logout_user(request):
    ...
    elif request.session.has_key('college_id'):
        del request.session['college_id']
        del request.session['college_email']
...
def myaccount(request):
    if request.session.has_key('college_id') or request.session.has_key('user_id'):
        user, type = (RegisteredUsers.objects.get(user_id = uuid.UUID(request.session['user_id'])), 'user') if (request.session.has_key('user_id') and RegisteredUsers.objects.filter(user_id = uuid.UUID(request.session['user_id']))) else (RegisteredCollege.objects.get(user_id = uuid.UUID(request.session['college_id'])), 'college')
        if type == 'user':
            return render(request, 'kollegecliq/user-dashboard.html', context={'user':user,'type':type})
        else:
            college_id = request.session['college_id']
            if CollegeBasicInfo.objects.filter(college__user_id=uuid.UUID(college_id)).exists():
                college_basic_info = CollegeBasicInfo.objects.get(college__user_id=uuid.UUID(college_id))
                is_info = True
            else:
                college_basic_info={}
                is_info = False
            if CollegeAffiliation.objects.filter(college__user_id=uuid.UUID(college_id)).exists():
                affiliaztions = CollegeAffiliation.objects.filter(college__user_id=uuid.UUID(college_id))
                is_affilitions = True
            else:
                affiliaztions = {}
                is_affilitions = False
            return render(request,'college/myaccount.html',context = {'college_id':college_id, 'basic_info':college_basic_info, 'is_info':is_info,
            'affiliations':affiliaztions, 'is_affiliation':is_affilitions})
    return render(request,'kollegecliq/login.html')
        # return render(request, 'kollegecliq/myaccount.html', context={'user':'9084086876','type':'college'})
    ...
    if request.method == "POST":
        if RegisteredUsers.objects.filter(username=request.POST['username'].lower()):
            ...
            if data['username'].isnumeric() or checkemail(data['username']):
                ...
                password = request.POST.get("login-password2",None)
                ...
                otp = generateOtp()
                print(otp)
                if request.session.has_key('user_username'):
                    del request.session['user_username']
                if request.session.has_key('userotp'):
...
def apply(request):
    ...
    return render(request,'kollegecliq/login.html', context={"state_path":request.path})
...
def verify_otp(request):
    ...
    if request.method == 'POST':
        ...
        if type == 'college':
            ...
            if request.session.has_key('userotp') and request.session['userotp'] == otp:
